...
We include 2,000 genes with the highest DS and get the `expression` matrix, where each row represents a sample tissue and each column represents a gene.
...
```{r}
# Function used to normalize each factor to have the l2-norm equal one
normFactor <- function(x) {
  return(x / sqrt(sum(x^2)))
}

FX <- apply(FX, MARGIN = 2, FUN = normFactor)
...
p <- ggplot(
  ...
  ggh4x::facet_grid2(Factor ~ ., scales = "free_y")
p
```
...
```{r}
# Normalize to have l2 norm equal 1 for each factor
